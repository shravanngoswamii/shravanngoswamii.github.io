...
export default function Card({ href, frontmatter, index = 1, collection }: Props) {
  ...
  return (
    <a href={href} title={title} className="group block w-full relative border-t border-skin-line/20 first:border-t-0">
      ...
      <div className="py-8 flex flex-col md:flex-row md:items-baseline gap-4 relative z-10 transition-colors duration-500 ease-out group-hover:bg-skin-fill/50">
        ...
        <div className="flex-1 min-w-0 pr-4 md:pr-8 transition-transform duration-500 ease-out group-hover:translate-x-2">
           ...
           <h3 className={`vt-${slugifyStr(title)} text-2xl sm:text-3xl font-serif text-skin-base mb-3 leading-tight transition-colors duration-300`}>
             {title}
           </h3>
